...
description: Как настроить визуальную презентацию и SEO для репозитория Yandex Disk
---

Этот документ описывает, как поддерживается визуальное представление репозитория **Yandex Disk** на GitHub и в поиске.
...
- **Badge "Music Assistant"** — динамический shields.io endpoint badge: `https://trudenboy.github.io/ma-provider-tools/badges/filesystem_yandex_disk.json`. Cron `update-ma-version-badges.yml` обновляет JSON каждые 4 часа и показывает, в какие каналы MA (stable / beta / dev) включена текущая версия провайдера.
