...
# Dev environment setup for the filesystem_yandex_disk provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/filesystem_yandex_disk"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: filesystem_yandex_disk → $TARGET"
  ...
  echo "Provider filesystem_yandex_disk linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
